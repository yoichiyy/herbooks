...
class BookNumButton extends ChangeNotifier {
  final int booknum = 0;
  final String musume = "";
...
//家計の方
  Future<void> kakeiCount(_controller, _category) async {
    final snapshot =
        await FirebaseFirestore.instance.collection('users').doc(uid).get();
    final userName = snapshot.data()!['name'];

    await FirebaseFirestore.instance.collection('kakei').doc().set({
      'amount': int.tryParse(_controller.value),
      "date": dailyCount,
      "month": monthlyCount,
      'category': _category,
      'user': userName,
...
//絵本の方
  Future<void> bookNumCount(booknum, musume) async {
